...
describe('Unit tests URLList', () => {
  ...
  it('displays only filtered URL', async () => {
    ...
    const filteredMock = {
      request: {
        query: GET_ALL_URLS,
        variables: { searchText: 'Test URL 1', sortField: '', currentPage: 1 },
      },
    ...
    render(
      <MemoryRouter>
        <MockedProvider mocks={[defaultMock, filteredMock]} addTypename={false}>
          <Routes>
            <Route path="/" element={<URLList />} />
          </Routes>
        </MockedProvider>
  ...
  it('displays URL when sortField is selected', async () => {
    const sortedMock = {
      request: {
        query: GET_ALL_URLS,
        variables: { searchText: '', sortField: 'name', currentPage: 1 },
      },
      result: { data: sortedUrls },
    };
    render(
      <MemoryRouter>
        <MockedProvider mocks={[defaultMock, sortedMock]} addTypename={false}>
          <Routes>
            <Route path="/" element={<URLList />} />
          </Routes>
        </MockedProvider>
  ...
  it('displays Page 2 URL when next page is clicked', async () => {
    ...
    render(
      <MemoryRouter>
        <MockedProvider mocks={[defaultMock, paginateMock]} addTypename={false}>
          <Routes>
            <Route path="/" element={<URLList />} />
          </Routes>
        </MockedProvider>
